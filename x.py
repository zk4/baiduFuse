...
from core.main_args  import mainArgs
encrpted_length = 512
...
#             logger.info(f'{error_map[str(jdata["error_code"])]} args: {path}')
    ...
    def _readDir(self,path,depth=2,threadPool=pool):
        if path not in self.traversed_folder :
            self.traversed_folder.set(path,b'1',expire=mainArgs.cache_timeout)
...
#         logger.info(f'open {path}')
        ...
        try:
            if path not in self.downloading_files:
                ...
                # logger.debug(f'file size ----------------------{self.buffer[path]["st_size"]}')
                x= Task(url,path,self.disk.getHeader(),self.buffer[path]['st_size'])
                x.start()
                ...
                if filename.startswith("enc."):
                    if offset ==0  :
                        if data and len(data)> encrpted_length:
                            data = bytes(cipher(data,0,encrpted_length,mainArgs.key))
                        else:
                            print("decrpt failed!")
                return data
        except Exception as e:
...
if __name__ == '__main__':
    ...
    FUSE(CloudFS(),mainArgs.mount,foreground=True,nonempty=False,async_read=True,raw_fi=True)
